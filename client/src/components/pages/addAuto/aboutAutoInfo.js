...
console.log(addAuto)
 ...
 const handleChange = e => {
  ...
  return(
    ...
    <p>Марка<select className="new_auto_form_mark" name='mark' value={addAuto.mark}  onChange={handleChange}>{Object.keys(cars.list).map(e =><option value={e}>{e}</option>)}</select></p>
    <p>Модель<select className="new_auto_form_model" name='model' value={addAuto.model}   onChange={handleChange}>{cars.list[addAuto.mark].map(e =><option value={e}>{e}</option>)}</select></p>
