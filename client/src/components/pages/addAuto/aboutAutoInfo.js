...
import { addAutoForm, kwtInPower } from '../../../../redux/actions/actions'
...
console.log(addAuto)
 ...
 const handleChange = e => {
  ...
  if (e.target.name=='power'){
    dispatch(kwtInPower(addAuto, e.target.value))
  }
  }
  ...
  return(
    ...
    <p>Марка<select className="new_auto_form_mark" name='mark' value={addAuto.mark}  onChange={handleChange}><option>Выберите марку автомобиля</option>{Object.keys(cars.list).map(e =><option value={e} key={e}>{e}</option>)}</select></p>
    ...
    <p>Мощность<input className="new_auto_form_power" placeholder='100 л.с.' name='power' value={addAuto.power} onChange={handleChange}/><input className="new_auto_form_powerkwt" placeholder='73,55' name='powerkwt'onChange={handleChange} value={addAuto.powerkwt}/></p>
