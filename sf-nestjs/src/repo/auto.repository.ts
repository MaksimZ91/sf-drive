...
export class AutoRepository {
  ...
  async filterAuto(startDate: string, endDate: string, type:string) {
    ...
    .createQueryBuilder('auto') 
    .leftJoinAndSelect('auto.arenda', 'arenda')
    .where('arenda.startDay  BETWEEN :begin AND :end', { begin: new Date(startDate), end: new Date(endDate)})
    .orWhere('arenda.endDay  BETWEEN :begin AND :end', { begin: new Date(startDate), end: new Date(endDate)})
    .orWhere(':begin  BETWEEN arenda.startDay AND arenda.endDay',{begin: new Date(startDate), end: new Date(endDate)})
    .orWhere(':end BETWEEN arenda.startDay AND arenda.endDay',{begin: new Date(startDate), end: new Date(endDate)})
